...
router.get("/properties", async (req, res) => {
    try {
        const properties = await Property.find(); // No callback, use await
        res.json(properties);
    } catch (error) {
...
router.post("/add-property", async (req, res) => {
    try {
        const newProperty = new Property(req.body); // Create a new Property instance with the request data
...
router.get("/filter-properties", async (req, res) => {
    try {
        ...
        if (req.query.demand) filters.demand = req.query.demand;
